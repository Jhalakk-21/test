...
def all_train_data_in():
    print("begin of all_train_data_in")
    allDataX = []
    allDataY = []
...
#                print(type(inX[1,1,1]))
                ...
                inY  = data['RGB']
                allDataY.append(inY)
    print("end of all_train_data_in")
            
    return allDataX, allDataY


def all_test_data_in():
    print("begin of all_test_data_in")
...
def PrepareDataAndiniValue(R,C,prepare='No'):
    DataRoad = 'CAVEdata/'
    DataRoad = os.path.abspath(DataRoad)
    print(DataRoad)
...
#    if not folder:
    if prepare != 'No':
        ...
        ML.mkdir(DataRoad+'Z/')
        n = 0
        # relative_path = 'CMHF-net/rowData/CAVEdata/complete_ms_data'
        ...
        #     print("path_exists")
        for root, dirs, files in os.walk("/mnt/d/sem7/ee798r/project/MHF-net/CMHF-net/rowdata/cavedata/complete_ms_data"):
            print("in os walk")
            for i in range(32):
                n=n+1
                Z = np.zeros([16,16,31])
...
#                else:
                X = readImofDir('/mnt/d/sem7/ee798r/project/MHF-net/CMHF-net/rowdata/cavedata/complete_ms_data'+dirs[Ind[i]-1]+'/'+dirs[Ind[i]-1])/255
                Y = np.tensordot(X,R,(2,0))
                for j in range(32):
                    for k in range(32):
                        Z = Z + X[j:512:32,k:512:32,:]*C[k,j]
                # sio.savemat(DataRoad+'X/'+dirs[Ind[i]-1], {'msi': X})  
                sio.savemat(os.path.join(DataRoad, 'X', dirs[Ind[i] - 1]), {'msi': X})  # Save the .mat file
                # sio.savemat(DataRoad+'Y/'+dirs[Ind[i]-1], {'RGB': Y}) 
                sio.savemat(os.path.join(DataRoad, 'Y', dirs[Ind[i] - 1]), {'RGB': Y})  # Save the .mat file

                # sio.savemat(DataRoad+'Z/'+dirs[Ind[i]-1], {'Zmsi': Z})   
                sio.savemat(os.path.join(DataRoad, 'Z', dirs[Ind[i] - 1]), {'Zmsi': Z})  # Save the .mat file
        ...
        allX = np.matrix(allX)
        allY = np.matrix(allY)
        # if np.linalg.matrix_rank(allY.T * allY) < (allY.T * allY).shape[0]:
        #     print("Matrix is singular or rank deficient.")
        # else:
        #     iniA = np.linalg.inv(allY.T * allY) @ (allY.T @ allX)

# Use the pseudo-inverse instead of the regular inverse
        iniA = np.linalg.pinv(allY.T @ allY) @ (allY.T @ allX)
        sio.savemat(os.path.join(DataRoad, 'iniA.mat'), {'iniA': iniA})
        sio.savemat(os.path.join(DataRoad, 'List.mat'), {'Ind': Ind})
        
        initemp = np.eye(31)
        iniUp1 = np.tile(initemp,[3,3,1,1])
        # sio.savemat(DataRoad+'iniUp', {'iniUp1': iniUp1}) 
        sio.savemat(os.path.join(DataRoad, 'iniUp.mat'), {'iniUp1': iniUp1})
